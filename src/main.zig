...
const ProfileState = struct {
    ...
    fn deinit(self: *ProfileState, allocator: std.mem.Allocator) void {
...
const NodeJson = struct {
    ...
    anytls_server: ?[]const u8 = null,
    anytls_port: ?[]const u8 = null,
    server_ip: ?[]const u8 = null,
    latency: ?[]const u8 = null,
    ping: ?[]const u8 = null,
};
...
const NodeRecord = struct {
    ...
    fn deinit(self: *NodeRecord, allocator: std.mem.Allocator) void {
        ...
        allocator.free(self.latency);
        allocator.free(self.ping);
...
fn parseArgs(args: []const []const u8) !Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        } else if (std.mem.eql(u8, arg, "--effective")) {
...
fn runStat(allocator: std.mem.Allocator, nodes: []NodeRecord, query: QueryOptions) !void {
    ...
    switch (format) {
        ...
        .summary => {
            const stdout = std.fs.File.stdout().deprecatedWriter();
            for (protocols.items, 0..) |item, idx| {
                if (idx > 0) try stdout.writeAll("；");
                try stdout.print("{s}：{d}个", .{ item.key, item.count });
            }
            if (protocols.items.len == 0) {
                try stdout.writeAll("无");
            }
            try stdout.writeAll("\n");
        },
        else => unreachable,
...
fn runSyncSource(allocator: std.mem.Allocator, state: *SchemaState, query: QueryOptions) !void {
    ...
    for (old_nodes) |node| {
        ...
        try old_source_nodes.append(allocator, .{
            ...
            .server_ip = try allocator.dupe(u8, node.server_ip),
            .latency = try allocator.dupe(u8, node.latency),
            .ping = try allocator.dupe(u8, node.ping),
            .schema = node.schema,
            .rev = node.rev,
...
fn resolveStatFormat(raw: ?[]const u8) !OutputFormat {
    ...
    if (std.ascii.eqlIgnoreCase(format, "json")) return .json;
    if (std.ascii.eqlIgnoreCase(format, "summary")) return .summary;
    return error.InvalidArguments;
}
...
fn buildLegacyNodeJsonAlloc(allocator: std.mem.Allocator, arena: std.mem.Allocator, draft: *LegacyNodeDraft, source_meta_rows: []const SourceMetaRow, ts: u64, stats: *LegacyMigrationStats) ![]u8 {
    ...
    try normalizeLegacyBoolField(arena, &draft.fields, "anytls_ai");
...
fn normalizeNodeDefaultFields(allocator: std.mem.Allocator, obj: *std.json.ObjectMap) !void {
    ...
    } else if (std.mem.eql(u8, node_type, "9")) {
        try normalizeLegacyDefaultField(allocator, obj, "anytls_ai", "0");
    }
}
...
fn keepLegacyTypeField(node_type: []const u8, key: []const u8) bool {
    ...
    if (std.mem.eql(u8, node_type, "9")) {
        return stringIn(key, &.{ "anytls_server", "anytls_port", "anytls_pass", "anytls_sni", "anytls_ai" });
    }
    return false;
}
...
fn stripNullJsonValue(allocator: std.mem.Allocator, value: *std.json.Value) !void {
    switch (value.*) {
        .object => |*obj| {
            var remove_keys = std.ArrayList([]const u8){};
            defer remove_keys.deinit(allocator);
            ...
            var it = obj.iterator();
            while (it.next()) |entry| {
                try stripNullJsonValue(allocator, entry.value_ptr);
        ...
        .array => |*arr| {
            for (arr.items) |*item| {
                try stripNullJsonValue(allocator, item);
            }
        },
        else => {},
    }
...
fn writePrettyJsonStripNullsFile(allocator: std.mem.Allocator, path: []const u8, content: []const u8) !void {
    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, content, .{});
    defer parsed.deinit();
    try stripNullJsonValue(allocator, &parsed.value);
    ...
    try writeFileAtomic(path, with_newline);
}

fn wrapUserOutboundJsonAlloc(allocator: std.mem.Allocator, user_json: []const u8, target_addr: []const u8, tag_name: []const u8) !?[]u8 {
    var arena_state = std.heap.ArenaAllocator.init(allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    var parsed = try std.json.parseFromSlice(std.json.Value, arena, user_json, .{});
    const root = &parsed.value;
    if (root.* != .object) return null;

    var outbound_ptr: ?*std.json.Value = null;
    if (root.object.getPtr("outbound")) |value_ptr| {
        outbound_ptr = value_ptr;
    } else if (root.object.getPtr("outbounds")) |list_ptr| {
        if (list_ptr.* == .array and list_ptr.array.items.len > 0) {
            outbound_ptr = &list_ptr.array.items[0];
        }
    }
    if (outbound_ptr == null or outbound_ptr.?.* != .object) return null;

    if (target_addr.len > 0) {
        const outbound_obj = &outbound_ptr.?.object;
        const protocol = if (outbound_obj.get("protocol")) |protocol_value|
            switch (protocol_value) {
                .string => protocol_value.string,
                else => "",
            }
        else
            "";
        if (std.mem.eql(u8, protocol, "vmess") or std.mem.eql(u8, protocol, "vless")) {
            if (outbound_obj.getPtr("settings")) |settings_ptr| {
                if (settings_ptr.* == .object) {
                    if (settings_ptr.object.getPtr("vnext")) |vnext_ptr| {
                        if (vnext_ptr.* == .array and vnext_ptr.array.items.len > 0 and vnext_ptr.array.items[0] == .object) {
                            try jsonPutString(arena, &vnext_ptr.array.items[0].object, "address", target_addr);
                        }
                    }
                }
            }
        } else if (std.mem.eql(u8, protocol, "socks") or std.mem.eql(u8, protocol, "shadowsocks") or std.mem.eql(u8, protocol, "trojan")) {
            if (outbound_obj.getPtr("settings")) |settings_ptr| {
                if (settings_ptr.* == .object) {
                    if (settings_ptr.object.getPtr("servers")) |servers_ptr| {
                        if (servers_ptr.* == .array and servers_ptr.array.items.len > 0 and servers_ptr.array.items[0] == .object) {
                            try jsonPutString(arena, &servers_ptr.array.items[0].object, "address", target_addr);
                        }
                    }
                }
            }
        } else if (std.mem.eql(u8, protocol, "hysteria")) {
            if (outbound_obj.getPtr("settings")) |settings_ptr| {
                if (settings_ptr.* == .object) {
                    try jsonPutString(arena, &settings_ptr.object, "address", target_addr);
                }
            }
        }
    }

    try jsonPutString(arena, &outbound_ptr.?.object, "tag", tag_name);
    return try renderCompactJsonAlloc(allocator, outbound_ptr.?.*);
}

fn allocJsonStringOrNull(allocator: std.mem.Allocator, value: ?[]const u8) ![]u8 {
    if (value) |inner| {
        if (inner.len > 0) return try std.fmt.allocPrint(allocator, "\"{s}\"", .{inner});
    }
    return try allocator.dupe(u8, "null");
}

fn allocJsonStringOrEmpty(allocator: std.mem.Allocator, value: ?[]const u8) ![]u8 {
    if (value) |inner| {
        if (inner.len > 0) return try std.fmt.allocPrint(allocator, "\"{s}\"", .{inner});
    }
...
fn writeExecutableFileAtomic(path: []const u8, content: []const u8) !void {
    var file = try std.fs.cwd().createFile(path, .{ .truncate = true, .mode = 0o755 });
    defer file.close();
    try file.writeAll(content);
}

fn upsertMetaValue(path: []const u8, key: []const u8, value: []const u8) !void {
    const content = try readFileAllocCompat(std.heap.c_allocator, path, max_skipd_frame);
...
fn parsePortU16(text: []const u8) ?u16 {
...
fn reservePortText(used: *std.AutoHashMap(u16, void), text: []const u8) !void {
    if (parsePortU16(text)) |port| {
        try used.put(port, {});
    }
}

fn reservePortsFromNetstat(allocator: std.mem.Allocator, used: *std.AutoHashMap(u16, void)) !void {
    const result = try std.process.Child.run(.{
        .allocator = allocator,
        .argv = &.{
            "sh",
            "-c",
            "netstat -an 2>/dev/null | awk '/^(tcp|udp|raw)/ { n = split($4, parts, \":\"); port = parts[n]; if (port ~ /^[0-9]+$/) print port }'",
        },
    ...
    while (lines.next()) |line| {
        try reservePortText(used, line);
    }
}

fn reservePortsFromMaterializeIndex(allocator: std.mem.Allocator, used: *std.AutoHashMap(u16, void), index_file: []const u8) !void {
    if (!fileExists(index_file)) return;
    const content = try readFileAllocCompat(allocator, index_file, max_skipd_frame);
    defer allocator.free(content);
    ...
    while (lines.next()) |line| {
        ...
        _ = parts.next();
        const port_text = parts.next() orelse "";
        try reservePortText(used, port_text);
    }
}

fn buildWebtestObfsStartPortMap(allocator: std.mem.Allocator, ids_file: []const u8, nodes: []const NodeRecord, index_file: []const u8) !std.StringHashMap(u16) {
    var port_map = std.StringHashMap(u16).init(allocator);
    errdefer port_map.deinit();

    const ids_content = try readFileAllocCompat(allocator, ids_file, max_skipd_frame);
    defer allocator.free(ids_content);

    var obfs_ids = std.ArrayList([]const u8){};
    defer obfs_ids.deinit(allocator);
    ...
    while (lines.next()) |raw_line| {
        ...
        try obfs_ids.append(allocator, node.id);
    }

    if (obfs_ids.items.len == 0) return port_map;

    var used = std.AutoHashMap(u16, void).init(allocator);
    defer used.deinit();
    try reservePortsFromMaterializeIndex(allocator, &used, index_file);
    try reservePortsFromNetstat(allocator, &used);

    const min_port: u32 = 2000;
    const max_port: u32 = 65000;
    const range: u32 = max_port - min_port + 1;
    var prng = std.Random.DefaultPrng.init(@as(u64, @intCast(std.time.microTimestamp())));
    const random = prng.random();
    var next_offset = random.uintLessThan(u32, range);

    for (obfs_ids.items) |node_id| {
        var assigned: ?u16 = null;
        var offset: u32 = 0;
        while (offset < range) : (offset += 1) {
            const candidate_u32 = min_port + ((next_offset + offset) % range);
            const candidate: u16 = @intCast(candidate_u32);
            if (used.contains(candidate)) continue;
            try used.put(candidate, {});
            try port_map.put(node_id, candidate);
            assigned = candidate;
            next_offset = (candidate_u32 - min_port + 1) % range;
            break;
        }
        if (assigned == null) return error.NoSpaceLeft;
    }
...
fn materializeRuntimeArtifactsFresh(allocator: std.mem.Allocator, socket_path: []const u8, nodes: []const NodeRecord, output_dir: []const u8, profile: RuntimeArtifactProfile) !usize {
    ...
    _ = try warmJsonCache(allocator, nodes);
...
fn warmWebtestArtifacts(allocator: std.mem.Allocator, socket_path: []const u8, nodes: []const NodeRecord) !usize {
    ...
    if (std.mem.trim(u8, build_content, " \t\r\n").len > 0) {
        var runtime_cfg = try loadWebtestRuntimeConfig(allocator, socket_path);
        defer runtime_cfg.deinit(allocator);
        var obfs_start_ports = try buildWebtestObfsStartPortMap(allocator, build_ids_file, nodes, index_file);
        defer obfs_start_ports.deinit();
        try buildWebtestNodeArtifactsWithShell(allocator, build_ids_file, nodes, cache_dir, node_dir, meta_dir, runtime_cfg, &obfs_start_ports);
    }
...
fn buildWebtestNodeArtifactsWithShell(allocator: std.mem.Allocator, ids_file: []const u8, nodes: []const NodeRecord, cache_dir: []const u8, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig, obfs_start_ports: *const std.StringHashMap(u16)) !void {
    ...
    var shell_reasons = std.StringHashMap([]const u8).init(allocator);
    defer shell_reasons.deinit();
    ...
    while (lines.next()) |raw_line| {
        ...
        if (findNodeIndexById(nodes, node_id)) |idx| {
            if (try buildWebtestNodeArtifactNative(allocator, nodes[idx], node_dir, meta_dir, runtime_cfg, obfs_start_ports)) {
                continue;
            }
            try shell_reasons.put(node_id, try webtestNativeFallbackReason(allocator, nodes[idx], obfs_start_ports.get(node_id)));
        } else {
            try shell_reasons.put(node_id, "node_not_found");
        }
    ...
    const script =
        ...
        "FSS_WEBTEST_CACHE_DIR=\"$2\"\n" ++
        "FSS_WEBTEST_CACHE_NODE_DIR=\"$3\"\n" ++
        "FSS_WEBTEST_CACHE_META_DIR=\"$4\"\n" ++
        "FSS_WEBTEST_CACHE_INDEX_FILE=\"$2/materialize_index.txt\"\n" ++
        "FSS_WEBTEST_CACHE_AGG_OUTBOUNDS_FILE=\"$2/all_outbounds.json\"\n" ++
        "FSS_WEBTEST_CACHE_GLOBAL_META_FILE=\"$2/cache.meta\"\n" ++
    ...
    const result = try std.process.Child.run(.{
        .allocator = allocator,
        .argv = &.{ "sh", "-c", script, "sh", shell_ids_file, cache_dir, node_dir, meta_dir },
    ...
    var shell_lines = std.mem.splitScalar(u8, shell_ids.items, '\n');
    while (shell_lines.next()) |raw_line| {
        const node_id = std.mem.trim(u8, raw_line, " \t\r\n");
        if (node_id.len == 0) continue;
        const reason = shell_reasons.get(node_id) orelse "native_fallback_unknown";
        const meta_path = try std.fmt.allocPrint(allocator, "{s}/{s}.meta", .{ meta_dir, node_id });
        defer allocator.free(meta_path);
        if (fileExists(meta_path)) {
            try upsertMetaValue(meta_path, "builder_reason", reason);
        }
    }
}
...
fn buildWebtestNodeArtifactNative(allocator: std.mem.Allocator, node: NodeRecord, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig, obfs_start_ports: *const std.StringHashMap(u16)) !bool {
    if (std.mem.eql(u8, node.type_id, "0")) {
        return try buildWebtestSsNative(allocator, node, node_dir, meta_dir, runtime_cfg, obfs_start_ports.get(node.id));
    }
    ...
    if (std.mem.eql(u8, node.type_id, "9")) {
        return try buildWebtestAnyTlsNative(allocator, node, node_dir, meta_dir, runtime_cfg, obfs_start_ports.get(node.id));
    }
    return false;
}

fn webtestNativeFallbackReason(allocator: std.mem.Allocator, node: NodeRecord, obfs_start_port: ?u16) ![]const u8 {
    if (std.mem.eql(u8, node.type_id, "0")) {
        const method = try extractStringFieldAlloc(allocator, node.raw_json, "method");
        defer if (method) |value| allocator.free(value);
        const password = try extractStringFieldAlloc(allocator, node.raw_json, "password");
        defer if (password) |value| allocator.free(value);
        if (method == null or method.?.len == 0 or password == null or password.?.len == 0) return "ss_missing_method_or_password";
        const ss_obfs = try extractStringFieldAlloc(allocator, node.raw_json, "ss_obfs");
        defer if (ss_obfs) |value| allocator.free(value);
        if (ss_obfs) |value| {
    ...
    if (std.mem.eql(u8, node.type_id, "3")) {
        ...
        if (use_json != null and std.mem.eql(u8, use_json.?, "1")) {
            const user_json = try extractStringFieldAlloc(allocator, node.raw_json, "v2ray_json");
            defer if (user_json) |value| allocator.free(value);
            if (user_json == null or user_json.?.len == 0) return "vmess_user_json_missing";
            const wrapped = try wrapUserOutboundJsonAlloc(allocator, user_json.?, node.server, "probe");
            if (wrapped == null) return "vmess_user_json_structure_unsupported";
        ...
        if (!std.mem.eql(u8, net, "tcp") and !std.mem.eql(u8, net, "ws") and !std.mem.eql(u8, net, "grpc") and !std.mem.eql(u8, net, "h2") and !std.mem.eql(u8, net, "quic") and !std.mem.eql(u8, net, "kcp")) return "vmess_network_unsupported";
        ...
        if (!std.mem.eql(u8, sec, "none") and !std.mem.eql(u8, sec, "tls")) return "vmess_security_unsupported";
        const uuid = try extractStringFieldAlloc(allocator, node.raw_json, "v2ray_uuid");
        defer if (uuid) |value| allocator.free(value);
        if (uuid == null or uuid.?.len == 0) return "vmess_uuid_missing";
        ...
        if (std.mem.eql(u8, net, "tcp") and headtype_tcp != null and std.mem.eql(u8, headtype_tcp.?, "http")) return "vmess_tcp_http_header_unsupported";
        return "native_supported";
    }
    if (std.mem.eql(u8, node.type_id, "4")) {
        const use_json = try extractStringFieldAlloc(allocator, node.raw_json, "xray_use_json");
        defer if (use_json) |value| allocator.free(value);
        if (use_json != null and std.mem.eql(u8, use_json.?, "1")) {
            const user_json = try extractStringFieldAlloc(allocator, node.raw_json, "xray_json");
            defer if (user_json) |value| allocator.free(value);
            if (user_json == null or user_json.?.len == 0) return "xray_user_json_missing";
            const wrapped = try wrapUserOutboundJsonAlloc(allocator, user_json.?, node.server, "probe");
            if (wrapped == null) return "xray_user_json_structure_unsupported";
            allocator.free(wrapped.?);
            return "native_supported";
        }
        const xray_prot = try extractStringFieldAlloc(allocator, node.raw_json, "xray_prot");
        defer if (xray_prot) |value| allocator.free(value);
        const prot = if (xray_prot) |value| value else "vless";
        if (!std.mem.eql(u8, prot, "vless") and !std.mem.eql(u8, prot, "vmess")) return "xray_protocol_unsupported";
        const network = try extractStringFieldAlloc(allocator, node.raw_json, "xray_network");
        defer if (network) |value| allocator.free(value);
        const net = if (network) |value| value else "tcp";
        if (!std.mem.eql(u8, net, "tcp") and !std.mem.eql(u8, net, "ws") and !std.mem.eql(u8, net, "grpc") and !std.mem.eql(u8, net, "xhttp") and !std.mem.eql(u8, net, "httpupgrade")) return "xray_network_unsupported";
        const headtype_tcp = try extractStringFieldAlloc(allocator, node.raw_json, "xray_headtype_tcp");
        defer if (headtype_tcp) |value| allocator.free(value);
        if (std.mem.eql(u8, net, "tcp") and headtype_tcp != null and std.mem.eql(u8, headtype_tcp.?, "http")) return "xray_tcp_http_header_unsupported";
        const security = try extractStringFieldAlloc(allocator, node.raw_json, "xray_network_security");
        defer if (security) |value| allocator.free(value);
        const sec = if (security) |value| value else "none";
        if (!std.mem.eql(u8, sec, "none") and !std.mem.eql(u8, sec, "tls") and !std.mem.eql(u8, sec, "reality")) return "xray_security_unsupported";
        const uuid = try extractStringFieldAlloc(allocator, node.raw_json, "xray_uuid");
        defer if (uuid) |value| allocator.free(value);
        if (uuid == null or uuid.?.len == 0) return "xray_uuid_missing";
        return "native_supported";
    }
    if (std.mem.eql(u8, node.type_id, "5")) {
        const password = try extractStringFieldAlloc(allocator, node.raw_json, "trojan_uuid");
        defer if (password) |value| allocator.free(value);
        if (password == null or password.?.len == 0) return "trojan_password_missing";
        return "native_supported";
    }
    if (std.mem.eql(u8, node.type_id, "8")) {
        const pass = try extractStringFieldAlloc(allocator, node.raw_json, "hy2_pass");
        defer if (pass) |value| allocator.free(value);
        if (pass == null or pass.?.len == 0) return "hy2_password_missing";
        return "native_supported";
    }
    if (std.mem.eql(u8, node.type_id, "9")) {
        const pass = try extractStringFieldAlloc(allocator, node.raw_json, "anytls_pass");
        defer if (pass) |value| allocator.free(value);
        if (pass == null or pass.?.len == 0) return "anytls_password_missing";
        if (obfs_start_port == null) return "anytls_port_unavailable";
        return "native_supported";
    }
    return "protocol_not_supported";
}

fn buildWebtestSsNative(allocator: std.mem.Allocator, node: NodeRecord, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig, obfs_start_port: ?u16) !bool {
    const ss_obfs = try extractStringFieldAlloc(allocator, node.raw_json, "ss_obfs");
    defer if (ss_obfs) |value| allocator.free(value);
    const ss_obfs_host = try extractStringFieldAlloc(allocator, node.raw_json, "ss_obfs_host");
    defer if (ss_obfs_host) |value| allocator.free(value);
    const method = try extractStringFieldAlloc(allocator, node.raw_json, "method");
    defer if (method) |value| allocator.free(value);
    const password = try extractStringFieldAlloc(allocator, node.raw_json, "password");
    defer if (password) |value| allocator.free(value);
    ...
    const obfs_mode = if (ss_obfs) |value|
        if (value.len > 0 and !std.mem.eql(u8, value, "0")) value else null
    else
        null;
    if (obfs_mode) |value| {
    ...
    const meta_path = try std.fmt.allocPrint(allocator, "{s}/{s}.meta", .{ meta_dir, node.id });
    defer allocator.free(meta_path);
    const start_path = try std.fmt.allocPrint(allocator, "{s}/{s}_start.sh", .{ node_dir, node.id });
    defer allocator.free(start_path);
    const stop_path = try std.fmt.allocPrint(allocator, "{s}/{s}_stop.sh", .{ node_dir, node.id });
    ...
    if (obfs_mode) |value| {
        ...
        defer allocator.free(remote_port_quoted);
        const obfs_quoted = try shellQuoteAlloc(allocator, value);
        defer allocator.free(obfs_quoted);
        const obfs_host_quoted = if (ss_obfs_host) |host|
            if (host.len > 0)
                try shellQuoteAlloc(allocator, host)
            else
                null
        else
            null;
        defer if (obfs_host_quoted) |host| allocator.free(host);
        const obfs_host_segment = if (obfs_host_quoted) |host|
            try std.fmt.allocPrint(allocator, " --obfs-host {s}", .{host})
        else
            try allocator.dupe(u8, "");
        defer allocator.free(obfs_host_segment);

        const start_script = try std.fmt.allocPrint(allocator, "#!/bin/sh\n" ++
            "_wt_root=${{WT_RUNTIME_ROOT:-$(cd \"$(dirname \"$0\")/..\" && pwd)}}\n" ++
            "\"${{_wt_root}}/wt-obfs\" -s {s} -p {s} -l {d} --obfs {s}{s}{s} >/dev/null 2>&1 &\n" ++
            "_i=20\n" ++
            "while [ ${{_i}} -gt 0 ]; do\n" ++
            "    netstat -nl 2>/dev/null | awk '{{print $4}}' | grep -E \"[:\\\\.]{d}$\" >/dev/null 2>&1 && break\n" ++
            "    usleep 100000\n" ++
            "    _i=$((_i - 1))\n" ++
            "done\n", .{
            server_quoted,
            ...
            obfs_quoted,
            obfs_host_segment,
            if (tcp_fast_open) " --fast-open" else "",
            obfs_start_port.?,
        });
        defer allocator.free(start_script);
        try writeExecutableFileAtomic(start_path, start_script);

        const stop_script = try std.fmt.allocPrint(allocator, "#!/bin/sh\n" ++
            "_pid=$(ps -w | grep \"wt-obfs\" | grep -w {s} | grep -w {s} | grep -w {d} | awk '{{print $1}}' | sed -n '1p')\n" ++
            "if [ -n \"${{_pid}}\" ]; then\n" ++
            "    kill -9 ${{_pid}}\n" ++
            "fi\n", .{
            server_quoted,
        ...
        defer allocator.free(stop_script);
        try writeExecutableFileAtomic(stop_path, stop_script);
    } else {
        std.fs.cwd().deleteFile(start_path) catch {};
        std.fs.cwd().deleteFile(stop_path) catch {};
    }

    const meta = try std.fmt.allocPrint(allocator, "node_type=0\n" ++
        "node_rev={d}\n" ++
        "linux_ver={s}\n" ++
        "ss_basic_tfo={s}\n" ++
...
fn buildWebtestVmessNative(allocator: std.mem.Allocator, node: NodeRecord, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig) !bool {
    ...
    if (use_json != null and std.mem.eql(u8, use_json.?, "1")) {
        const user_json = try extractStringFieldAlloc(allocator, node.raw_json, "v2ray_json");
        defer if (user_json) |value| allocator.free(value);
        if (user_json == null or user_json.?.len == 0) return false;
        const tag_name = try std.fmt.allocPrint(allocator, "proxy{s}", .{node.id});
        ...
        const wrapped = try wrapUserOutboundJsonAlloc(allocator, user_json.?, node.server, tag_name);
        if (wrapped == null) return false;
        defer allocator.free(wrapped.?);
        try writeFileAtomic(out_path, wrapped.?);
    ...
    const http_json = if (std.mem.eql(u8, net, "h2")) h2: {
    ...
    const quic_json = if (std.mem.eql(u8, net, "quic")) quic: {
    ...
    const kcp_json = if (std.mem.eql(u8, net, "kcp")) kcp: {
        ...
        break :kcp try std.fmt.allocPrint(
            allocator,
...
fn buildWebtestXrayNative(allocator: std.mem.Allocator, node: NodeRecord, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig) !bool {
    const use_json = try extractStringFieldAlloc(allocator, node.raw_json, "xray_use_json");
    defer if (use_json) |value| allocator.free(value);
    if (use_json != null and std.mem.eql(u8, use_json.?, "1")) {
        const user_json = try extractStringFieldAlloc(allocator, node.raw_json, "xray_json");
    ...
    defer if (xray_prot) |value| allocator.free(value);
    const prot = if (xray_prot) |value| value else "vless";
    if (!std.mem.eql(u8, prot, "vless") and !std.mem.eql(u8, prot, "vmess")) return false;

    const network = try extractStringFieldAlloc(allocator, node.raw_json, "xray_network");
    defer if (network) |value| allocator.free(value);
    const net = if (network) |value| value else "tcp";
    if (!std.mem.eql(u8, net, "tcp") and !std.mem.eql(u8, net, "ws") and !std.mem.eql(u8, net, "grpc") and !std.mem.eql(u8, net, "xhttp") and !std.mem.eql(u8, net, "httpupgrade")) return false;
    const headtype_tcp = try extractStringFieldAlloc(allocator, node.raw_json, "xray_headtype_tcp");
    defer if (headtype_tcp) |value| allocator.free(value);
    if (std.mem.eql(u8, net, "tcp") and headtype_tcp != null and std.mem.eql(u8, headtype_tcp.?, "http")) return false;
    ...
    defer if (security) |value| allocator.free(value);
    const sec = if (security) |value| value else "none";
    if (!std.mem.eql(u8, sec, "none") and !std.mem.eql(u8, sec, "tls") and !std.mem.eql(u8, sec, "reality")) return false;

    const uuid = try extractStringFieldAlloc(allocator, node.raw_json, "xray_uuid");
    defer if (uuid) |value| allocator.free(value);
    if (uuid == null or uuid.?.len == 0) return false;
    const encryption = try extractStringFieldAlloc(allocator, node.raw_json, "xray_encryption");
    defer if (encryption) |value| allocator.free(value);
    const flow = try extractStringFieldAlloc(allocator, node.raw_json, "xray_flow");
    defer if (flow) |value| allocator.free(value);
    const host = try extractStringFieldAlloc(allocator, node.raw_json, "xray_network_host");
    defer if (host) |value| allocator.free(value);
    const path = try extractStringFieldAlloc(allocator, node.raw_json, "xray_network_path");
    defer if (path) |value| allocator.free(value);
    const grpc_authority = try extractStringFieldAlloc(allocator, node.raw_json, "xray_grpc_authority");
    defer if (grpc_authority) |value| allocator.free(value);
    const grpc_mode = try extractStringFieldAlloc(allocator, node.raw_json, "xray_grpc_mode");
    defer if (grpc_mode) |value| allocator.free(value);
    ...
    defer if (alpn_http) |value| allocator.free(value);
    const fingerprint = try extractStringFieldAlloc(allocator, node.raw_json, "xray_fingerprint");
    defer if (fingerprint) |value| allocator.free(value);
    const pcs = try extractStringFieldAlloc(allocator, node.raw_json, "xray_pcs");
    defer if (pcs) |value| allocator.free(value);
    const vcn = try extractStringFieldAlloc(allocator, node.raw_json, "xray_vcn");
    defer if (vcn) |value| allocator.free(value);
    const publickey = try extractStringFieldAlloc(allocator, node.raw_json, "xray_publickey");
    defer if (publickey) |value| allocator.free(value);
    const shortid = try extractStringFieldAlloc(allocator, node.raw_json, "xray_shortid");
    defer if (shortid) |value| allocator.free(value);
    const spiderx = try extractStringFieldAlloc(allocator, node.raw_json, "xray_spiderx");
    defer if (spiderx) |value| allocator.free(value);
    const show = try extractStringFieldAlloc(allocator, node.raw_json, "xray_show");
    defer if (show) |value| allocator.free(value);
    const xhttp_mode = try extractStringFieldAlloc(allocator, node.raw_json, "xray_xhttp_mode");
    defer if (xhttp_mode) |value| allocator.free(value);
    ...
    const sni_json = try allocJsonStringOrNull(allocator, if (effective_sni.len > 0) effective_sni else null);
    defer allocator.free(sni_json);
    const fingerprint_json = try allocJsonStringOrEmpty(allocator, if (fingerprint != null and fingerprint.?.len > 0) fingerprint.? else "chrome");
    defer allocator.free(fingerprint_json);
    const pcs_json = try allocJsonStringOrEmpty(allocator, pcs);
    defer allocator.free(pcs_json);
    const vcn_json = try allocJsonStringOrEmpty(allocator, vcn);
    defer allocator.free(vcn_json);
    const publickey_json = try allocJsonStringOrNull(allocator, publickey);
    defer allocator.free(publickey_json);
    const shortid_json = try allocJsonStringOrEmpty(allocator, shortid);
    defer allocator.free(shortid_json);
    const spiderx_json = try allocJsonStringOrEmpty(allocator, spiderx);
    defer allocator.free(spiderx_json);

    const encryption_value = if (encryption != null and encryption.?.len > 0) encryption.? else "none";
    const flow_enabled = !(std.mem.eql(u8, sec, "none") and (!std.mem.eql(u8, prot, "vless") or std.mem.eql(u8, encryption_value, "none")));
    const flow_json = if (flow_enabled and flow != null and flow.?.len > 0)
        try allocJsonStringOrNull(allocator, flow.?)
    else
        try allocator.dupe(u8, "null");
    defer allocator.free(flow_json);
    const user_json = if (std.mem.eql(u8, prot, "vless"))
        try std.fmt.allocPrint(
            allocator,
            "{{\"id\":\"{s}\",\"encryption\":\"{s}\",\"flow\":{s}}}",
            .{
                uuid.?,
                encryption_value,
                flow_json,
        ...
        try std.fmt.allocPrint(
            ...
            .{
                uuid.?,
                if (encryption != null and encryption.?.len > 0 and !std.mem.eql(u8, encryption.?, "none")) encryption.? else "auto",
            },
        );
    ...
    const reality_json = if (std.mem.eql(u8, sec, "reality"))
        try std.fmt.allocPrint(
            allocator,
            "{{\"show\":{s},\"fingerprint\":{s},\"serverName\":{s},\"publicKey\":{s},\"shortId\":{s},\"spiderX\":{s}}}",
            .{
                if (show != null and std.mem.eql(u8, show.?, "1")) "true" else "false",
                fingerprint_json,
                sni_json,
                publickey_json,
                shortid_json,
    ...
    defer allocator.free(grpc_json);
    const xhttp_json = if (std.mem.eql(u8, net, "xhttp")) xhttp: {
        const path_json = try allocJsonStringOrEmpty(allocator, path);
        defer allocator.free(path_json);
        const host_json = try allocJsonStringOrEmpty(allocator, host);
        defer allocator.free(host_json);
        break :xhttp try std.fmt.allocPrint(allocator, "{{\"path\":{s},\"host\":{s},\"mode\":\"{s}\"}}", .{
            path_json,
            host_json,
            if (xhttp_mode) |value| value else "auto",
        });
    } else try allocator.dupe(u8, "null");
    defer allocator.free(xhttp_json);
    const httpupgrade_json = if (std.mem.eql(u8, net, "httpupgrade")) httpupgrade: {
        const path_json = try allocJsonStringOrEmpty(allocator, path);
        defer allocator.free(path_json);
        const host_json = try allocJsonStringOrEmpty(allocator, host);
        defer allocator.free(host_json);
        break :httpupgrade try std.fmt.allocPrint(allocator, "{{\"path\":{s},\"host\":{s}}}", .{ path_json, host_json });
    } else try allocator.dupe(u8, "null");
    defer allocator.free(httpupgrade_json);
    const tcp_json = try allocator.dupe(u8, "null");
    defer allocator.free(tcp_json);
    ...
    const out_json = try std.fmt.allocPrint(
        ...
        "{{\n" ++
            "  \"tag\": \"proxy{s}\",\n" ++
            "  \"protocol\": \"{s}\",\n" ++
            "  \"settings\": {{\"vnext\": [{{\"address\": \"{s}\", \"port\": {s}, \"users\": [{s}]}}]}},\n" ++
            "  \"streamSettings\": {{\"network\": \"{s}\", \"security\": \"{s}\", \"tlsSettings\": {s}, \"xtlsSettings\": null, \"realitySettings\": {s}, \"tcpSettings\": {s}, \"kcpSettings\": null, \"wsSettings\": {s}, \"httpSettings\": null, \"quicSettings\": null, \"grpcSettings\": {s}, \"httpupgradeSettings\": {s}, \"xhttpSettings\": {s}, \"sockopt\": {{\"tcpFastOpen\": {s}}}}},\n" ++
            "  \"mux\": {{\"enabled\": false}}\n" ++
            "}}\n",
        .{
            node.id,
...
fn anytlsServerAddrAlloc(allocator: std.mem.Allocator, server: []const u8, port: []const u8) ![]u8 {
    if (std.mem.indexOfScalar(u8, server, ':') != null and !(std.mem.startsWith(u8, server, "[") and std.mem.endsWith(u8, server, "]"))) {
        return try std.fmt.allocPrint(allocator, "[{s}]:{s}", .{ server, port });
    }
    return try std.fmt.allocPrint(allocator, "{s}:{s}", .{ server, port });
}

fn buildWebtestAnyTlsNative(allocator: std.mem.Allocator, node: NodeRecord, node_dir: []const u8, meta_dir: []const u8, runtime_cfg: WebtestRuntimeConfig, start_port_opt: ?u16) !bool {
    const start_port = start_port_opt orelse return false;
    const anytls_pass = try extractStringFieldAlloc(allocator, node.raw_json, "anytls_pass");
    defer if (anytls_pass) |value| allocator.free(value);
    if (anytls_pass == null or anytls_pass.?.len == 0) return false;
    const anytls_sni = try extractStringFieldAlloc(allocator, node.raw_json, "anytls_sni");
    defer if (anytls_sni) |value| allocator.free(value);
    const anytls_ai = try extractStringFieldAlloc(allocator, node.raw_json, "anytls_ai");
    defer if (anytls_ai) |value| allocator.free(value);
    const out_path = try std.fmt.allocPrint(allocator, "{s}/{s}_outbounds.json", .{ node_dir, node.id });
    defer allocator.free(out_path);
    const meta_path = try std.fmt.allocPrint(allocator, "{s}/{s}.meta", .{ meta_dir, node.id });
    defer allocator.free(meta_path);
    ...
    const stop_path = try std.fmt.allocPrint(allocator, "{s}/{s}_stop.sh", .{ node_dir, node.id });
    defer allocator.free(stop_path);
    const pass_path = try std.fmt.allocPrint(allocator, "{s}/{s}_anytls.pass", .{ node_dir, node.id });
    defer allocator.free(pass_path);
    const pid_path = try std.fmt.allocPrint(allocator, "{s}/{s}_anytls.pid", .{ node_dir, node.id });
    defer allocator.free(pid_path);
    const port_text = try std.fmt.allocPrint(allocator, "{d}", .{start_port});
    defer allocator.free(port_text);
    const server_addr = try anytlsServerAddrAlloc(allocator, node.server, node.port);
    defer allocator.free(server_addr);
    const server_addr_q = try shellQuoteAlloc(allocator, server_addr);
    defer allocator.free(server_addr_q);
    const pass_path_q = try shellQuoteAlloc(allocator, pass_path);
    defer allocator.free(pass_path_q);
    const pid_path_q = try shellQuoteAlloc(allocator, pid_path);
    defer allocator.free(pid_path_q);
    const listen_addr = try std.fmt.allocPrint(allocator, "127.0.0.1:{s}", .{port_text});
    defer allocator.free(listen_addr);
    const listen_q = try shellQuoteAlloc(allocator, listen_addr);
    defer allocator.free(listen_q);
    const sni_arg = blk: {
        const sni_value = if (anytls_sni) |value| value else "";
        if (sni_value.len == 0) break :blk try allocator.dupe(u8, "");
        const sni_q = try shellQuoteAlloc(allocator, sni_value);
        defer allocator.free(sni_q);
        break :blk try std.fmt.allocPrint(allocator, " -sni {s}", .{sni_q});
    };
    defer allocator.free(sni_arg);
    const verify_arg = if (anytls_ai != null and std.mem.eql(u8, anytls_ai.?, "1"))
        try allocator.dupe(u8, " --insecure")
    else
        try allocator.dupe(u8, " --verify");
    defer allocator.free(verify_arg);
    ...
    const out_json = try std.fmt.allocPrint(
        ...
        "{{\n" ++
            "  \"tag\": \"proxy{s}\",\n" ++
            "  \"protocol\": \"socks\",\n" ++
            "  \"settings\": {{\"servers\": [{{\"address\": \"127.0.0.1\", \"port\": {s}}}]}},\n" ++
    ...
    defer allocator.free(out_json);
    try writeFileAtomic(out_path, out_json);
    try writeFileAtomic(pass_path, anytls_pass.?);

    const start_script = try std.fmt.allocPrint(
        allocator,
        "#!/bin/sh\n" ++
            "pid_file={s}\n" ++
            "if [ -s \"$pid_file\" ]; then kill \"$(cat \"$pid_file\")\" >/dev/null 2>&1 || true; rm -f \"$pid_file\"; fi\n" ++
            "\"/koolshare/bin/anytls-zig\" -server {s} --password-file {s} -l {s}{s}{s} >/dev/null 2>&1 &\n" ++
            "echo $! > \"$pid_file\"\n",
        .{ pid_path_q, server_addr_q, pass_path_q, listen_q, sni_arg, verify_arg },
    );
    defer allocator.free(start_script);
    try writeExecutableFileAtomic(start_path, start_script);

    const stop_script = try std.fmt.allocPrint(
        allocator,
        "#!/bin/sh\n" ++
            "pid_file={s}\n" ++
            "if [ -s \"$pid_file\" ]; then kill \"$(cat \"$pid_file\")\" >/dev/null 2>&1 || true; rm -f \"$pid_file\"; fi\n",
    ...
    const meta = try std.fmt.allocPrint(
        allocator,
        "node_type=9\nnode_rev={d}\nlinux_ver={s}\nss_basic_tfo={s}\nserver_resolv_mode={s}\nserver_resolver={s}\nbuilder=native\nbuilder_reason=native\nhas_start=1\nhas_stop=1\nstart_port={s}\nbuilt_at={d}\n",
        .{ node.rev, runtime_cfg.linux_ver, runtime_cfg.tfo, runtime_cfg.resolv_mode, runtime_cfg.resolver, port_text, std.time.timestamp() },
    );
    defer allocator.free(meta);
    try writeFileAtomic(meta_path, meta);
    return true;
...
fn writeWebtestGlobalMeta(allocator: std.mem.Allocator, socket_path: []const u8, path: []const u8, ids: []const []const u8, profile: []const u8) !void {
    ...
    const content = try std.fmt.allocPrint(allocator, "cache_rev=1\n" ++
...
fn parseNodeRecordFromDecodedOwnedJson(allocator: std.mem.Allocator, owned_json: []u8, fallback_id: []const u8) !NodeRecord {
    ...
    return .{
        ...
        .server_ip = try allocator.dupe(u8, value.server_ip orelse ""),
        .latency = try allocator.dupe(u8, value.latency orelse ""),
        .ping = try allocator.dupe(u8, value.ping orelse ""),
...
fn loadProfileStatesAlloc(allocator: std.mem.Allocator) ![]ProfileState {
    const runtime_path = "/tmp/upload/ss_subscribe_profiles.json";
    const file = std.fs.openFileAbsolute(runtime_path, .{}) catch return try allocator.alloc(ProfileState, 0);
    defer file.close();
    const contents = try file.readToEndAlloc(allocator, max_skipd_frame);
    defer allocator.free(contents);
    if (contents.len == 0) return try allocator.alloc(ProfileState, 0);

    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, contents, .{});
    defer parsed.deinit();
    if (parsed.value != .object) return try allocator.alloc(ProfileState, 0);
    const items = parsed.value.object.get("items") orelse return try allocator.alloc(ProfileState, 0);
    if (items != .array) return try allocator.alloc(ProfileState, 0);

    var out = std.ArrayList(ProfileState){};
    errdefer {
    ...
    for (items.array.items) |item| {
        ...
        const enabled_value = item.object.get("enabled") orelse continue;
        if (id_value != .string or enabled_value != .bool) continue;
        try out.append(allocator, .{
            .id = try allocator.dupe(u8, id_value.string),
            .enabled = enabled_value.bool,
        });
    }
    return try out.toOwnedSlice(allocator);
}

fn freeProfileStates(allocator: std.mem.Allocator, items: []ProfileState) void {
    for (items) |*item| item.deinit(allocator);
    allocator.free(items);
}

fn profileEnabled(profile_states: []const ProfileState, profile_id: []const u8) bool {
    if (profile_id.len == 0) return true;
    for (profile_states) |item| {
        if (std.mem.eql(u8, item.id, profile_id)) return item.enabled;
    }
    return true;
}
...
fn filterNodeSliceInPlace(allocator: std.mem.Allocator, nodes: *[]NodeRecord, query: QueryOptions) !void {
    ...
    const profile_states = if (query.effective) try loadProfileStatesAlloc(allocator) else try allocator.alloc(ProfileState, 0);
    defer freeProfileStates(allocator, profile_states);
    ...
    for (nodes.*) |node| {
        ...
        if (query.effective and !profileEnabled(profile_states, node.profile_id)) continue;
        try kept.append(allocator, node);
    }
...
fn protocolInfo(type_id: []const u8, xray_prot: []const u8) struct { short: []const u8, label: []const u8 } {
    ...
    if (std.mem.eql(u8, type_id, "9")) return .{ .short = "anytls", .label = "AnyTLS" };
...
fn effectiveEndpoint(value: NodeJson) struct { server: []const u8, port: []const u8 } {
    if (value.type) |type_id| {
        ...
        if (std.mem.eql(u8, type_id, "9")) {
            return .{ .server = value.anytls_server orelse "", .port = value.anytls_port orelse "" };
        }
    }
...
fn renderNodeViewJsonAlloc(allocator: std.mem.Allocator, node: NodeRecord) ![]u8 {
    ...
    try writer.writeAll(",\"server_ip\":");
    try writeJsonString(writer, node.server_ip);
    try writer.writeAll(",\"latency\":");
    try writeJsonString(writer, node.latency);
    try writer.writeAll(",\"ping\":");
    try writeJsonString(writer, node.ping);
    try writer.writeAll(",\"identity\":");
...
fn printCommandUsage(command: Command, writer: anytype) !void {
    switch (command) {
        ...
        .stat => try writer.writeAll("Usage: node-tool stat [--ids 1,2,3] [--source user|subscribe] [--source-tag tag] [--profile-id value] [--effective] [--airport-identity value] [--protocol type] [--name keyword] [--identity value] [--format text|json|summary] [--socket path]\n"),
        ...
        .webtest_groups => try writer.writeAll("Usage: node-tool webtest-groups --output-dir /tmp/fancyss_webtest [--ids 1,2,3 | --ids-file /tmp/ids.txt | --source user|subscribe | --source-tag tag | --profile-id value | --effective | --airport-identity value | --group name | --protocol type | --name keyword | --identity value] [--socket path]\n"),
        ...
        .warm_cache => try writer.writeAll("Usage: node-tool warm-cache [--env] [--json] [--direct-domains] [--webtest] [--ids 1,2,3 | --ids-file /tmp/ids.txt] [--source user|subscribe] [--source-tag tag] [--profile-id value] [--effective] [--airport-identity value] [--group name] [--protocol type] [--name keyword] [--identity value] [--socket path]\n"),
